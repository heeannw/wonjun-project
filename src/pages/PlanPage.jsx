...
export default function PlanPage() {
  ...
  const plannedDays = currentMonthDates.filter((date) => plans[date]).length
  const doneDays = currentMonthDates.filter((date) => logs[date]).length
  const totalDone = currentMonthDates.reduce((sum, date) => sum + (logs[date]?.total_distance_m || 0), 0)
  const completionRate = plannedDays ? Math.round((doneDays / plannedDays) * 100) : 0

  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-xl font-bold text-white">월간 훈련 스케줄</h1>
          <p className="text-slate-400 text-sm mt-0.5">한 달 훈련 계획과 실제 수행 기록을 캘린더로 관리하세요</p>
        </div>
      </div>
        ...
        <div className="px-5 py-4 border-b border-slate-700/40 flex items-center justify-between">
          <div className="flex items-baseline gap-4">
            ...
            <div className="flex items-center gap-4 text-sm">
              <span className="text-slate-500">계획 <span className="text-blue-400 font-semibold">{plannedDays}일</span></span>
              <span className="text-slate-500">실행 <span className="text-green-400 font-semibold">{totalDone.toLocaleString()}m</span></span>
              <span className="text-slate-500">완료율 <span className="text-white font-semibold">{completionRate}%</span></span>
            </div>
          </div>
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 px-4">
          <div className="bg-[#1a1d27] rounded-2xl p-6 w-full max-w-md border border-slate-700/50 shadow-2xl">
            <div className="flex items-center justify-between mb-5">
              <div>
                <p className="text-slate-400 text-xs">훈련 계획</p>
            ...
            <div>
              <label className="block text-sm text-slate-400 mb-1.5">계획 메모</label>
              <textarea
                value={editForm.custom_note}
                onChange={(e) => setEditForm({ custom_note: e.target.value })}
                rows={5}
                placeholder={'예: 장거리 지구력\n목표 7,000m / 페이스 유지 / 마무리 스트레칭'}
